...
class TestFiles:
    def test_create_file(self, files_client: FilesClient):
        ...
        response_data = CreateFileResponseSchema.model_validate_json(response.text)  # Тут десериализуем JSON строку в модель

        assert_status_code(actual=response.status_code, expected=HTTPStatus.OK)
        assert_create_file_response(request=request, response=response_data)
        validate_json_schema(instance=response.json(), schema=response_data.model_json_schema())  # Из готовой модели генерируем JsonSchema
